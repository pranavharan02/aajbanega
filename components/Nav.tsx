  ...
  return (
    <>
      ...
      <nav className="flex items-center justify-between py-4">
        <Link href="/dashboard" className="text-[22px] sm:text-[28px] font-extrabold tracking-tight text-[#2D2A26] leading-none">
          आज क्या बनेगा?
        </Link>
          ...
          <div className="flex justify-around items-center max-w-[680px] mx-auto px-2 pt-2 pb-[max(8px,env(safe-area-inset-bottom))]">
            {links.map((link) => {
              ...
              return (
                ...
                >
                  <span className={active ? 'text-[#2D2A26]' : 'text-[#8C8680]'}>{link.icon}</span>
                  <span className={`text-[11px] font-semibold ${active ? 'text-[#2D2A26]' : 'text-[#8C8680]'}`}>{link.label}</span>
                </Link>
              )
            })}
          </div>
